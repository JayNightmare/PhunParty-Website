...
describe("Join page WS-first flows", () => {
    it("shows a WS-delivered question immediately even if REST isstarted is false", async () => {
        ...
        // The WS question should render promptly
        await waitFor(() =>
            expect(
                screen.getByText(/WS Prompt arrives early/i)
            ).toBeInTheDocument()
        );
    });
    ...
    it("falls back to REST current question when WS question is absent and game started", async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText(/REST provided prompt/i)
            ).toBeInTheDocument()
        );
    });
});
